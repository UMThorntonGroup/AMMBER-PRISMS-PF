...
const ParaboloidSystem system = []()
{
  prisms::ConditionalOStreams::pout_base() << "Loading System...\n";
...
const Scales input_scales = Scales(
  []()
  {
    std::ifstream ifs("system.json");
    if (!ifs.is_open())
    ...
    nlohmann::json scale_parameters;
    ifs >> scale_parameters;
    return Scales(scale_parameters.at("dimensions"));
  }());
