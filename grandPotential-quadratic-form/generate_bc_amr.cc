...
main(int argc, char *argv[])
{
  ...
  if (set_gradient_amr)
    {
      if (system.length_scale == 0.0)
        {
          std::cout << "Warning: length_scale is zero, cannot set gradient AMR "
                       "correctly. This will be fixed with PRISMS-PF 3.0."
                    << std::endl;
        }
      for (unsigned int comp_index = 0; comp_index < system.num_comps; ++comp_index)
        {
